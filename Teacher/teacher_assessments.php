...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add_assessment') {
        ...
        if ($title === '') {
            ...
            $assignSectionId = !empty($_POST['assign_section_id']) ? (int)$_POST['assign_section_id'] : null;
            $assignStudentId = !empty($_POST['assign_student_id']) ? (int)$_POST['assign_student_id'] : null;
            
            if ($assignSectionId || $assignStudentId) {
                $stmtAssign = $conn->prepare('INSERT INTO assessment_assignments (assessment_id, section_id, student_id) VALUES (?, ?, ?)');
                $stmtAssign->bind_param('iii', $assessmentId, $assignSectionId, $assignStudentId);
                $stmtAssign->execute();
                ...
                if ($assignSectionId) {
                    createNotificationForSection(
                        ...
                        "Your teacher has created a new assessment: \"$title\". Check the My Tests section to take it.",
                        $assessmentId
                    );
                } elseif ($assignStudentId) {
...
$assessments = $conn->query("
    SELECT a.*, 
           ...
           GROUP_CONCAT(DISTINCT s.name SEPARATOR ', ') as assigned_sections,
           GROUP_CONCAT(DISTINCT st.name SEPARATOR ', ') as assigned_students
    FROM assessments a 
    LEFT JOIN assessment_assignments aa ON aa.assessment_id = a.id
    LEFT JOIN sections s ON aa.section_id = s.id
    LEFT JOIN students st ON aa.student_id = st.id
        ...
        <div class="card-body">
            <form method="POST">
                ...
                <div style="margin: 20px 0; padding: 16px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
                    <h4 style="margin: 0 0 16px 0; color: #374151;"><i class="fas fa-users"></i> Assign to Students/Sections (Optional)</h4>
                    <div class="grid grid-2">
                        <div>
                            <label>Assign to Section</label>
                            <select name="assign_section_id">
                    ...
                    <p style="margin: 8px 0 0 0; font-size: 0.9em; color: #6b7280;">
                        💡 <strong>Tip:</strong> You can assign assessments now or later from the "Schedule / Assign" page. 
                        If you don't assign now, students won't see the assessment until you assign it.
                    </p>
                </div>
            ...
            <table>
                <thead><tr><th>Title</th><th>Created</th><th>Assignment Status</th><th>Actions</th></tr></thead>
                <tbody>
                <?php if ($assessments && $assessments->num_rows > 0): while ($a = $assessments->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <?php if ($a['assignment_count'] > 0): ?>
                                <span style="color: #10b981; font-size: 0.9em;">
                                    <i class="fas fa-check-circle"></i> Assigned
                                    <?php if ($a['assigned_sections']): ?>
                                        <br><small>Section: <?php echo h($a['assigned_sections']); ?></small>
                                    <?php endif; ?>
                                    <?php if ($a['assigned_students']): ?>
                                        <br><small>Student: <?php echo h($a['assigned_students']); ?></small>
                                    <?php endif; ?>
                                </span>
                            <?php else: ?>
                                <span style="color: #f59e0b; font-size: 0.9em;">
                                    <i class="fas fa-exclamation-triangle"></i> Not Assigned
                                    <br><small>Students won't see this</small>
                                </span>
                            <?php endif; ?>
                        </td>
                        <td>
                            <a href="teacher_schedule.php" class="btn btn-secondary" style="margin-right: 8px; font-size: 0.9em;">
...
function prepareFormSubmission() {
    ...
    for (let block of questionBlocks) {
        ...
        if (sourceSelect.value === 'new') {
            ...
            // Check if the question bank selection container exists and has a selected question
            const bankSelection = block.querySelector('[id^="question-bank-selection-"]');
            
            // Debug logging
            console.log('Validation for from_bank:');
            console.log('- bankSelection:', bankSelection);
            console.log('- bankSelection innerHTML:', bankSelection ? bankSelection.innerHTML.substring(0, 200) : 'null');
            
            // Check if we have a selected question (either by hidden input or visual indicator)
            const hasSelectedQuestion = bankSelection && (
                bankSelection.innerHTML.includes('✓ SELECTED') || 
                bankSelection.querySelector('input[name*="[question_id]"]')
            );
            
            console.log('- hasSelectedQuestion:', hasSelectedQuestion);
